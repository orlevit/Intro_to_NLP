...
def average_vecs(words_pos_dict):
    for word, word_values in words_pos_dict.items():
        for pos, pos_values in word_values.items():
            avg_vecs = np.mean(words_pos_dict[word][pos]['wp_count'], axis=0)

            if isinstance(avg_vecs, list):
               avg_vecs = avg_vecs[0]

            words_pos_dict[word][pos]['wp_count'] = avg_vecs
            
            if len(pos_values['right_pos']):
                for right_pos, right_poses_values in pos_values['right_pos'].items():
                    avg_vecs = np.mean(right_poses_values['right_count'], axis=0)

                    if isinstance(avg_vecs, list):
                       avg_vecs = avg_vecs[0]

                    words_pos_dict[word][pos]['right_pos'][right_pos]['right_count'] = avg_vecs
                
                    if len(right_poses_values['left_pos']):
                        for rl_pos, rl_poses_values in right_poses_values['left_pos'].items():
                            avg_vecs = np.mean(rl_poses_values['left_count'], axis=0)

                            if isinstance(avg_vecs, list):
                               avg_vecs = avg_vecs[0]

                            words_pos_dict[word][pos]['right_pos'][right_pos]['left_pos'][rl_pos]['left_count'] = avg_vecs
                
            if len(pos_values['left_pos']):
                for left_pos, left_poses_values in pos_values['left_pos'].items(): 
                    avg_vecs = np.mean(left_poses_values['left_count'], axis=0)

                    if isinstance(avg_vecs, list):
                       avg_vecs = avg_vecs[0]

                    words_pos_dict[word][pos]['left_pos'][left_pos]['left_count'] = avg_vecs
...
def make_dictonary(train_location):
    ...
    for file_line in tqdm(file_lines):
        ...
        for ii, _ in enumerate(splitted_line):
            ...
            if ii  + 1 < len_line:
                _, right_pos = split_token_pos(splitted_line, ii + 1)
                add_right_pos_to_dict(words_pos_dict, token, pos, right_pos, t2v)
    
            if ii - 1 <= 0:
                _, left_pos = split_token_pos(splitted_line, ii - 1)
